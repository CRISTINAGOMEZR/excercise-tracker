...
export default function Heatmap({ counts, weeks = 18 }: Props) {
  // Base: hoy en día LOCAL, igual que se guardan los registros (ver ymd()).
  // Se ancla al mediodía para que sumar días no se descuadre con el cambio de hora.
  const now = new Date();
  const hoy = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 12);
  const dow = hoy.getDay(); // 0=Dom

  // Inicio = domingo de hace (weeks-1) semanas.
  const start = new Date(hoy);
  start.setDate(hoy.getDate() - dow - (weeks - 1) * 7);
  ...
  for (let w = 0; w < weeks; w++) {
    ...
    for (let d = 0; d < 7; d++) {
      ...
      const future = date.getTime() > hoy.getTime();
