...
import { addDays, hoy } from './dates';
import type { ActividadGuardada, Exercise, Registro, Rutina, RutinaItem } from '@/types';

// ─── Helpers ──────────────────────────────────────────────────────────────────

/** Hoy en `YYYY-MM-DD` local — nunca UTC (ver `lib/dates.ts`). */
...
export async function getRachaActual(): Promise<number> {
  ...
  let offset = 0;
  if (fechas[0] !== hoyStr) {
    if (fechas[0] !== addDays(hoyStr, -1)) return 0;
    offset = 1;
  }

  let racha = 0;
  for (let i = 0; i < fechas.length; i++) {
    if (fechas[i] === addDays(hoyStr, -(i + offset))) racha++;
    else break;
  }
  return racha;
}
...
export async function getTotalSemana(): Promise<number> {
  const desde = addDays(todayStr(), -6);
  const snap = await getDocs(
